...
public class FirstFragment_3 extends BaseFragment {
    ...
    public int setOnCreateView() {
        return R.layout.fragment_first_3;
    }
...
//        explistview.setOnGroupClickListener(new GroupClickListener());
    ...
    private void setMap(List<PinyinOrBiHuaBean.ResultBean> pinyinBeans) {
        for(PinyinOrBiHuaBean.ResultBean bihua:pinyinBeans){
            if(!keyData.contains(bihua.getBihua())){
                keyData.add(bihua.getBihua());
            }
        }
        for(String key:keyData){
            List<PinyinOrBiHuaBean.ResultBean> childrenData = new ArrayList<>();
            for(PinyinOrBiHuaBean.ResultBean bihua:pinyinBeans){
                if(key.equals(bihua.getBihua())){
                    childrenData.add(bihua);
                }
            }
        ...
        adapter = new PinnedHeaderExpandableAdapter(keyData,groupData, mContext,explistview,MainFirstFragment.FRAGMENT_BU_SHOU);
